...
const string strMessageMagic = "Hendercoin Signed Message:\n";
...
{
    int64 nSubsidy = 579000000 * COIN;

    // Subsidy is cut in half every 888 blocks

    nSubsidy >>= (nHeight / 888);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // Hendercoin: difficulty updates every 4 hours
static const int64 nTargetSpacing = 60; // Hendercoin: 1 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal hendercoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Hendercoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
